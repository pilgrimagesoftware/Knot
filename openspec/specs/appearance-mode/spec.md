...
Defines how the stored `Settings::appearance_mode` preference resolves to the
appearance the `knot` app actually paints, and when that resolution is re-run:
at startup, on an operating-system appearance change, and when the user picks
a different mode.
