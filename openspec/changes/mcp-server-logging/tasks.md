...
- [x] 1.1 Add the log constants to `knot-mcp/src/consts.rs` — rotation size cap, retained rolled-
      file count, heartbeat interval, log file name — and verify `cargo build --workspace` succeeds.
- [x] 1.2 Add the level and subject enums with `Display` (no stringly-typed vocabulary) and the
      `Entry` type carrying timestamp, level, subject and message; verify unit tests assert each
      variant's rendering.
- [x] 1.3 Implement entry formatting — UTC RFC 3339 timestamp with a sub-second component, then
...
- [x] 2.1 Implement the writer task owning the file: create the directory if absent, open for
...
- [x] 2.2 Implement the `Logger` handle — cheap `Clone`, holding an unbounded sender — and verify a
      test asserts sending does not block and that entries reach the file in order.
- [x] 2.3 Implement rotation: roll before the write that would exceed the cap, shift rolled files
...
- [x] 2.4 Implement the failure episode: report the first failure to stderr, stay silent for
...
- [x] 2.5 Verify `make lint` and `make size-check` pass for `knot-mcp` and that its `mod.rs`/
