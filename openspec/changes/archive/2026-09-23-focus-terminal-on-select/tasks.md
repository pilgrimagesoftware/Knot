...
- [x] 1.1 Add `FocusTarget` to `composer_focus.rs` and widen
      `showing_composer` to return it, with `has_live_grid` added to
      `SelectedAgentFacts`; verify `make build` passes
- [x] 1.2 Extend `tests/composer_focus.rs` with the new spec scenarios - a
