...
- Split-pane / multiple simultaneous visible surfaces per workspace window
  (tracked separately, see `plans/split-pane.md`).
- GPU-shader terminal rendering effects (cursor trails, background blur) -
  this is a plain cell-grid renderer using GPUI's own text/shape drawing.
