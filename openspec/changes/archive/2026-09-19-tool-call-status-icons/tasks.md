...
- [x] 1.1 Add a `status_icon(&str) -> Option<IconName>` mapping
...
- [x] 1.2 Verify the colour decision reuses `card_outline` +
...
- [x] 2.1 Replace the header's `status_label` text with the status icon where
      ...
      check.
- [x] 2.2 Confirm the header row's layout still truncates a long title
