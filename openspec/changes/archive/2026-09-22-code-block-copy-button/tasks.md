...
- [x] 1.1 Add `panel.copy_code` ("Copy code") and `panel.copied_code` ("Copied
...
- [x] 1.2 `touch crates/knot-core/src/lib.rs` before running the l10n tests, so
      the locale file is re-embedded rather than read from a stale build
      artifact; verify `cargo test -p knot-core` passes

## 2. The copy control

- [x] 2.1 Add a pure `pub(crate) fn code_block_copy_text(block: &CodeBlock) ->
...
- [x] 2.2 Add a private `code_block_actions` renderer to `markdown_view.rs`
      ...
      knot` succeeds
- [x] 2.3 Wire it into the `markdown_view` constructor with
...
- [x] 2.4 Confirm the new imports are explicit per the crate's convention
...
- [x] 3.1 In `crates/knot/src/tests/markdown_view.rs`, add tests for
...
- [x] 3.2 Extend that test module's doc comment to record what stays untested
