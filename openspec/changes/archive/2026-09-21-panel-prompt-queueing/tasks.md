...
- [x] 1.1 Add `queued_prompts: VecDeque<String>` and `enqueue_prompt`,
...
- [x] 1.2 Extend the `TurnEnd` arm: pending permission defers promotion;
...
- [x] 1.3 Add `take_pending_delivery` returning the owed prompt and clearing
      the one-shot. Unit test that it returns each value exactly once.

## 2. Delivery flush

- [x] 2.1 In `panel_session`'s drain task, after applying each event, read
...
- [x] 2.2 Confirm the permission gate at delivery: a pending permission leaves
      the flag unissued, and once `answer_permission` resolves the queue's
      promotion and subsequent delivery proceed.

## 3. The composer

- [x] 3.1 Loosen `send_panel_prompt`: drop the `!turn_active` requirement,
...
- [x] 3.2 Update `can_send` to `!blocked && !empty` so the Send control stays
      enabled through a turn. Verify the control enables mid-turn and still
      disables while a permission prompt is pending.
- [x] 3.3 Render the `queued` presentation in `panel_view` (muted bubble with
