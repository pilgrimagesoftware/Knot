...
- [x] 2.2 Build the Move to Workspace and Markdown Files submenus from the
      same sources the context menu uses. Verify in the app that both list
      what the context menu lists for the same agent.

## 3. Enablement

- [x] 3.1 Register the handlers on the workspace window while an agent is
      ...
      manager is focused.
- [x] 3.2 Route each action to `run_agent_menu_action` for the selected
...
- [x] 4.1 Rebuild the menu bar when the facts behind the dynamic submenus
...
- [x] 5.2 Confirm a dialog opened from the menu bar appears: the handler runs
