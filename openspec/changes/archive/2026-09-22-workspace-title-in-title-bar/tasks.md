...
- [x] 2.1 Turn `WorkspaceWindow::sidebar_title_bar` into a method and have it
...
- [x] 2.2 Update the doc comment on `sidebar_title_bar` — it currently explains
...
- [x] 2.3 Confirm a long workspace name truncates on one line and does not
...
- [x] 3.1 Add a `titled_as: String` field to `WorkspaceWindow`, initialised from
      ...
      `make lint`
- [x] 3.2 In `render`, call `window.set_window_title` when the resolved name
      ...
      the new name
- [x] 3.3 Call `cx.refresh_windows()` after a successful `rename_workspace` in
...
- [x] 4.2 Walk the spec's scenarios against the running app: two open windows
