...
- [x] 1.1 Archive `acp-agent-panel-ui` so `acp-panel-ui` exists under
...
- [x] 2.1 Add the three outline colours to `PanelStyle` - danger, info and
...
- [x] 2.2 Add a pure function from a card's status to which of the three
...
- [x] 3.1 Render the title in `PanelStyle`'s monospace family and the status
      in the proportional one. Verify in the app that a shell command reads
      as a command and "Done" beside it does not.
- [x] 3.2 Apply the outline colour from 2.2. Verify in the app across a turn
