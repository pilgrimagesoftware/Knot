...
- [x] 1.1 Add `tool_call_title: Option<String>` to `PermissionRequest` and
...
- [x] 1.2 Confirm existing `PermissionRequest` construction sites and finders
      compile with the additive field (it is `Option`, so nothing should
      break); run `cargo test -p knot-acp`.

## 2. Name resolution

- [x] 2.1 Add a pure `display_name` resolution on `PanelState` (request
      ...
      with no card at all.
- [x] 2.2 Wire `render_permission_prompt` to format the resolved name into
