...
- [x] 1.1 Create the single `WorkingIndicator` view/component in the gpui-kit
      shared UI layer and verify `cargo +nightly fmt` + `cargo clippy` pass on
      the workspace
- [x] 1.2 Add the shared status→state mapping (Working / Idle / Awaiting input /
      Error / not-running → indicator state) in one module and verify unit
      tests cover every mapping + the not-running case
- [x] 1.3 Implement the indicator's animation via gpui-kit's timer/animation
...
- [x] 2.1 Render the indicator on the agent row beside the existing state dot
      and verify it updates live when `activity-detection`'s status changes
- [x] 2.2 Add the "not running" visual state and verify a not-running agent
...
- [x] 3.1 Render the same indicator on the dashboard's agent card and verify
      it stays in lockstep with the sidebar row (no drift between surfaces)
- [x] 3.2 Wire the card's indicator to the shared component and verify the
      per-agent status transitions (Working/Idle/Awaiting input/Error) all
      show correctly on the card

## 4. Verification

- [x] 4.1 Run `openspec validate` on the change and verify all artifacts pass
