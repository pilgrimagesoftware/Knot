...
- [ ] 1.1 Add `issue` module (and `REPO` arg shape) to `knot-forge` exposing
...
- [ ] 2.2 Add `bug_report.*` keys to `crates/knot-core/locales/en.yml` for the
...
- [ ] 3.1 Create `bug_report/diagnostics.rs` composing the selectable
...
- [ ] 3.2 Add pure formatting helpers (localized labels via
...
- [ ] 4.1 Create `bug_report/form.rs` with subject/description state and pure
...
- [ ] 4.2 Create `bug_report/mod.rs` and `register_report_bug_action` wiring
...
- [ ] 4.3 Handle the "no window to attach to" case by logging and no-oping
      rather than crashing; verify a test with no windows runs without panic

## 5. Submission dispatch (`crates/knot`)

- [ ] 5.1 Create `bug_report/submit.rs` with `submit_with_gh`: probe
      ...
      retry-after-failure
- [ ] 5.2 Add `submit_via_browser`: build the pre-filled compose URL
...
- [ ] 5.3 Ensure the URL builder is one pure function with both dispatch
...
- [ ] 6.1 Add a `ReportBug` action and a Help-menu
...
- [ ] 7.1 Run the full gate and confirm it passes:
      `make fmt-check`, `make size-check`, `make lint` (clippy `-D warnings`),
      `make test`, `make build`
- [ ] 7.2 Read the `bug-reporting` and `app-menu` specs against the
