...
- [x] 1.1 In `crates/knot-agent-launch/src/registration.rs`, add tests
...
- [x] 2.1 Replace the `knot_instructions` format string: order the duties,
      ...
      pass.
- [x] 2.2 Update the doc comment on `knot_instructions` to record why the
...
- [x] 3.1 Rewrite `instructions_tell_an_agent_to_hand_work_to_its_knot` and
...
- [x] 4.1 Add "Knot instructions given to a launched agent" as this change's
      delta against `agent-launch-command`, to be promoted into
      `openspec/specs/` at archive. Verify `openspec validate --strict` passes.

## 5. Gate

- [x] 5.1 Run `make` and confirm the full gate passes.
