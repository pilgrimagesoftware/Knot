...
- [x] 2.3 Add `artifact_panel_open(id)` reading `markdown_file` and
...
- [x] 3.1 Give `render_markdown_pane` and `render_mermaid_pane` in
...
- [x] 3.2 Draw the chevron in each section's header when collapsible, toggling
...
- [x] 3.2a Keep each section's own close control in its header alongside the
      chevron, closing only that section's artifact; verify with a test
      covering "Closing one section leaves the other".
- [x] 3.3 Add the localization keys for the panel title, the expand and collapse
...
- [x] 4.1 Implement the toolbar in `artifact_panel/render.rs` - localized title,
...
- [x] 4.2 Render the single-section case: the one open section filling the panel,
      no divider, no collapse control; verify with a render test covering the
      spec's "Only a diagram is open".
- [x] 4.3 Render the dual-section case with `v_resizable`, feeding the two
...
- [x] 4.4 Render the collapsed cases as a plain `v_flex` with no resizable group
...
- [x] 5.0 Add an artifact-state landing to `repaint_poll_tick`'s `if` chain
...
- [x] 5.1 Remove the markdown and diagram arms from the pane selection in
...
- [x] 5.2 Replace `with_git_panel` with a `with_side_panels` that adds whichever
...
- [x] 5.3 Wire the panel's width through the group's `on_resize` to the clamped
      ...
      same width.
- [x] 5.4 Give the content panel the minimum width from 1.2 as its `size_range`
...
- [x] 6.1 Remove `has_markdown` and `has_diagram` from
      ...
      passes.
- [x] 6.1a Add the expanded check inside `focus_showing_pane`, beside the
...
- [x] 6.2 Update `crates/knot/src/tests/pane_focus.rs` - including
