...
- [x] 1.1 Add `crates/knot/src/workspace_window/terminal_font.rs` with
...
- [x] 1.2 Add unit tests for `resolve` covering an installed name, an
...
- [x] 2.1 Add the memo type to `terminal_font.rs` (the name that was asked
...
- [x] 2.2 Add the `terminal_font` field to `WorkspaceWindow` in
      `window.rs`, initialised wherever the window is constructed, and verify
      `make build` passes

## 3. Moving the call sites onto the memo

- [x] 3.1 Refresh the memo from `prepare_frame` in `render/mod.rs`, ahead of
      `resize_session_to_pane`
- [x] 3.2 Replace `terminal_font_family(&self.settings, cx)` with the memo
      read in `terminal_input.rs` (`resize_session_to_pane`, `grid_position`)
      and `render/content.rs` (`render_grid`, the scroll listener)
- [x] 3.3 Delete `terminal_font_family` from `chrome.rs` and verify
