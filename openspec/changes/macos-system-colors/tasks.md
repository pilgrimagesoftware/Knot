...
- [x] 1.1 Add `crates/knot/src/macos/mod.rs` and
...
- [x] 1.2 Convert those to GPUI `Hsla`; add the non-macOS stub returning the
...
- [x] 2.1 In `app_support.rs`, derive the theme accents (`primary*`,
...
- [x] 2.2 Map neutral chrome to the resolved neutrals: root/window background,
...
- [x] 3.1 In `panel_view`, drive the user prompt bubble from the system
...
- [x] 3.2 Confirm focused controls in the panel paint their border/ring with
      the accent (audit inputs, buttons and any element drawing a focus
      border for hardcoded focus colors; swap any found).
- [x] 3.3 Hook palette re-ingestion into the existing appearance observer so
