...
- [ ] 1.1 Add an `mcp_url: Option<&str>` (or equivalent) parameter to
...
- [ ] 1.2 Thread the knot MCP URL from `Settings` (`mcp_server_enabled`,
...
- [ ] 1.3 Add/extend an `AcpSession` or `AcpClient` test that starts a fake
...
- [ ] 2.1 Delete `AdapterLaunch.mcp_config` and `mcp_config_args` in
      ...
      leftover references.
- [ ] 2.2 Update `plan_launch` so `LaunchPlan::Adapter` carries just the
...
- [ ] 3.1 Rewrite `build_agent_command` in
...
- [ ] 3.2 Delete `mcp_and_registration_args`, the non-shell arms of
...
- [ ] 3.3 Rewrite `build_initialization_command` to the shell-only wrapper
      (no `KNOT_AGENT_ID`) per the new "Shell agent initialization wrapper"
      requirement, and update/replace its tests accordingly.
- [ ] 3.4 Simplify `plan_launch` so it only returns `LaunchPlan::Terminal`
...
- [ ] 4.1 In `AgentStore::create`/`edit` (`crates/knot-agents/src/store/`),
...
- [ ] 4.2 Remove the Terminal/Panel view-mode toggle button and its
...
- [ ] 4.3 Remove now-dead Terminal-mode rendering/session-management code
