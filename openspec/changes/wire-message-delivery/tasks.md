...
- [x] 1.1 Thread `Arc<Mutex<MessageStore>>` from `app_bootstrap` to
...
- [x] 2.1 Add per-agent last-nudged bookkeeping to the workspace window, and a
...
- [x] 2.2 Call it from the repaint poll and send `CHECK_INBOX_PROMPT` through
...
- [x] 2.3 Verify the deferred case in the app: message an agent that is
...
- [x] 3.2 Confirm a shell agent is never nudged, per `mcp-messaging`'s rule
