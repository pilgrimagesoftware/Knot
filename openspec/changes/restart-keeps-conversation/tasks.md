...
- [ ] 1.1 Add `AgentStore::restart_keeping_conversation`. Verify with
...
- [ ] 2.1 Add `AgentMenuEntry::RestartWithNewConversation`, its label, its
...
- [ ] 2.2 Add `WorkspaceWindow::restart_agents(ids, keep_conversation, cx)` and
  route Restart Agent, Restart All and Restart with New Conversation through
  it. The first two read `restore_conversation_on_launch` when confirmed.
...
- [ ] 2.3 Move the restart confirmations to l10n, with keep, clear and
