...
- [ ] 1.1 Add an `Import` tab to the settings window, with a section per
...
- [ ] 1.2 Add the shared result type an import returns - added, skipped as
...
- [ ] 2.1 Define the provider trait (does this tool have definitions; what are
...
- [ ] 2.2 Implement the Claude provider against `~/.claude/agents/*.md` and a
      ...
      not guessed at.
- [ ] 2.3 Import selected definitions as `user` personas, skipping any whose
...
- [ ] 3.1 Add the `plist` dependency and a reader for the `com.kochava.skwad`
...
- [ ] 3.2 Implement workspace import: bring the selected workspaces, the
...
- [ ] 3.4 Confirm the import does not write to Skwad: capture the plist's
