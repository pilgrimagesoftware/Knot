...
- [ ] 1.1 Add an `ActivationMode` enum (`Active`, `Passive`) to `knot-core`,
      ...
      trips.
- [ ] 1.2 Add `activation_mode` to `knot_agents::Agent` (durable, carried
...
- [ ] 1.3 Add `activation_mode` to `CreateOptions`, defaulting to `Passive`,
...
- [ ] 1.4 Add `activation_mode` to `EditRequest` and apply it in
...
- [ ] 2.1 Add `AgentStore::set_activated(id, bool)` and have `create` mark an
      `Active` agent activated at creation, so it starts when its workspace
      next opens. Verify with a test covering both modes.
- [ ] 2.2 Extract the session teardown that `WorkspaceWindow::remove_agent`
      ...
      tests are untouched.
- [ ] 2.3 Add `WorkspaceWindow::deactivate_agent(id)`: deactivate owned
...
- [ ] 2.4 Gate `ensure_session` and `ensure_panel_session` on the agent being
...
- [ ] 2.5 Mark an agent activated when its sidebar row is selected. Verify in
      the running app that a passive agent starts on first selection and a
      deactivated one restarts on the next.

## 3. What the user sees

- [ ] 3.1 Add the stopped placeholder to the content pane for a selected
