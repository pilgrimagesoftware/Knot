...
use crate::agents::registry::visible_agent_infos;
...
/// The crew list: every agent the caller may see, carrying its registry
/// fields. Workspace order, not the registry's ranking - this is the list
/// the user arranged, and `describe-agents` is the one that ranks.
fn list_agents_response(store: &AgentStore, caller_id: Uuid) -> Vec<AgentInfo> {
    visible_agent_infos(store, caller_id)
}
