...
pub fn agent_not_found(store: &AgentStore, provided_id: &str) -> ToolCallResult {
    ...
    if agents.is_empty() {
        return ToolCallResult::error(format!("Agent ID '{provided_id}' not found. No agents are currently available. Ask the user to check if Knot is running correctly."));
    }

    let mut message = format!("Agent ID '{provided_id}' not found. You may have forgotten your ID due to context loss. Here are all agents - find yourself by matching your working directory:\n\n");
    for agent in agents {
