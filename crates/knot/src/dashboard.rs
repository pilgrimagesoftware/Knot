...
pub(crate) fn sort_picker(
    ...
    on_select: impl Fn(DashboardSort, &mut gpui_kit::Window, &mut gpui_kit::App) + Clone + 'static,
) -> impl IntoElement {
...
fn agent_card(
    agent: &DashboardAgent, muted: gpui_kit::Hsla,
    ...
    v_flex()
        .id(gpui_kit::ElementId::from(format!(
            "dashboard-agent-card-{id}"
        )))
        .cursor_pointer()
        .w(px(CARD_WIDTH))
        .h(px(CARD_HEIGHT))
        ...
        .children(
            git_stats
                ...
                .map(|stats| crate::app_state::diff_stats_row(&stats, muted).text_xs()),
        )
...
fn add_agent_tile(
    ...
    h_flex()
        .id(gpui_kit::ElementId::from(format!(
            "dashboard-add-agent-{workspace_id}"
        )))
        .cursor_pointer()
        .w(px(CARD_WIDTH))
        .h(px(CARD_HEIGHT))
...
pub(crate) fn workspace_section(
    workspace: DashboardWorkspace, is_global: bool, muted: gpui_kit::Hsla,
    ...
    let color: gpui_kit::Hsla = gpui_kit::Rgba::try_from(workspace.color_hex.as_str())
        .map(Into::into)
        .unwrap_or_else(|_| rgb(0x1B4FB2).into());
            ...
            h_flex()
                ...
                .children(workspace.agents.iter().map(|agent| {
                    agent_card(agent, muted, {
                        let on_agent_tap = on_agent_tap.clone();
                        move |id, window, cx| on_agent_tap(id, window, cx)
                    })
                }))
