...
impl WorkspaceWindow {
    ...
    fn ensure_panel_session(&mut self, id: Uuid) {
        ...
        let Some(adapter) = knot_agent_launch::acp_adapter(&agent.agent_type) else {
            return;
        };
        ...
        let plan = knot_agent_launch::plan_launch(
            knot_core::ViewMode::Panel,
            Some(adapter),
            &self.settings,
            &request,
        );
        let knot_agent_launch::LaunchPlan::Adapter(adapter_launch) = plan else {
    ...
    fn render_panel_pane(
        ...
        match &*slot_guard {
            panel_session::PanelSessionSlot::Connecting => div()
                .size_full()
                .flex()
                .items_center()
                .justify_center()
                ...
                .into_any_element(),
            panel_session::PanelSessionSlot::Failed(message) => div()
                .size_full()
                .p_4()
    ...
    fn render_panel_input_area(
        ...
        expanded: bool, blocked: bool, turn_active: bool,
...
impl WorkspaceWindow {
    fn selected_agent_header(&self) -> Option<SelectedAgentHeader> {
        ...
        Some(SelectedAgentHeader {
            ...
            has_acp_adapter: knot_agent_launch::acp_adapter(&agent.agent_type).is_some(),
        })
    }
}
...
mod tests {
    ...
    fn agent_context_menu_omits_companion_actions_for_companions() {
        assert_eq!(
            agent_context_menu_items(false),
            vec![
                "Edit Agent…",
                "New Shell Companion",
                "Restart Agent",
