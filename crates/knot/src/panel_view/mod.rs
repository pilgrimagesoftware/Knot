...
impl PanelCallbacks {
    pub(crate) fn new(
        on_permission_decision: impl Fn(PermissionDecision) + 'static,
        on_toggle_track: impl Fn() + 'static, on_toggle_tool_call: impl Fn(String) + 'static,
        on_manual_scroll: impl Fn() + 'static,
    ) -> Self {
        Self {
            on_permission_decision: Rc::new(on_permission_decision),
...
pub(crate) fn render_panel(
    state: Arc<Mutex<PanelState>>, list: ListState, style: &PanelStyle, callbacks: PanelCallbacks,
) -> impl IntoElement {
...
/// neighbouring rows.
fn render_row(
    index: usize, state: &PanelState, style: &PanelStyle, list: &ListState,
...
fn render_message(
    ctx: Message<'_>, message: &PanelMessage, callbacks: &PanelCallbacks,
) -> gpui_kit::AnyElement {
    let Message {
        state,
        index,
    ...
    match message {
        ...
        PanelMessage::Assistant(text) => {
            v_flex()
                ...
                .child(div().w_full().min_w_0().child(TextView::markdown(
        ...
        PanelMessage::ToolCall(card) => render_tool_call_card(
            card,
            style,
            state.is_collapsed(card),
            callbacks.on_toggle_tool_call.clone(),
        )
        .into_any_element(),
...
fn render_response_actions(
    text: String, user_index: Option<usize>, index: usize, list: &ListState,
    on_manual_scroll: Rc<dyn Fn()>,
) -> impl IntoElement {
...
fn render_tool_call_card(
    card: &ToolCallCard, style: &PanelStyle, collapsed: bool, on_toggle: Rc<dyn Fn(String)>,
) -> impl IntoElement {
    ...
    v_flex()
        ...
        .child(
            h_flex()
                .id(("panel-tool-call-header", element_id(&card.id)))
                .w_full()
                .min_w_0()
                .gap_2()
                .items_center()
                ...
                .child(
                    Icon::new(disclosure_icon(collapsed))
                        .xsmall()
                        .text_color(rgb(MUTED)),
                )
                .child(
                    Icon::new(tool_call_icon(&card.kind))
                        .xsmall()
                        .text_color(rgb(MUTED)),
                )
                .child({
...
fn render_tool_call_content(
    content: &knot_acp::ToolCallContent, style: &PanelStyle,
) -> gpui_kit::AnyElement {
    match content {
        knot_acp::ToolCallContent::Diff {
            path,
            old_text,
            new_text,
...
/// addition.
fn render_diff(
    path: &str, old_text: Option<&str>, new_text: &str, style: &PanelStyle,
) -> impl IntoElement {
    v_flex()
        .w_full()
        .min_w_0()
