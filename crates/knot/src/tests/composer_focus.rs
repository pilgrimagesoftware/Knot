//! Where keyboard focus goes when a Panel-mode agent is selected
...
//! [`showing_composer`], the pure helper that says which agent's composer a
//! frame will draw.
...
use crate::workspace_window::composer_focus::SelectedAgentFacts;
use crate::workspace_window::composer_focus::showing_composer;
...
fn an_activated_panel_agent_shows_its_composer() {
    ...
    assert_eq!(showing_composer(false, Some(&agent)),
               Some(agent.id),
...
#[test]
fn a_terminal_mode_agent_shows_no_composer() {
    let agent = SelectedAgentFacts { is_panel_mode: false,
                                     ..panel_agent() };
    assert_eq!(showing_composer(false, Some(&agent)),
               None,
               "the terminal grid has no composer, and focusing the pane itself is this change's \
                stated non-goal");
}
...
fn a_takeover_view_shows_no_composer() {
    let agent = panel_agent();
    assert_eq!(showing_composer(true, Some(&agent)),
               None,
...
fn no_selection_shows_no_composer() {
    assert_eq!(showing_composer(false, None),
               None,
...
fn closing_a_markdown_pane_shows_the_composer_again() {
    ...
    let with_markdown = SelectedAgentFacts { has_markdown: true,
                                             ..panel_agent() };
    assert_eq!(showing_composer(false, Some(&with_markdown)), None);
    assert_eq!(showing_composer(false, Some(&agent)),
               Some(agent.id),
